...
class TestReserveProduct:
    @pytest.mark.asyncio
    async def test_reserve_product(self, async_client: httpx.AsyncClient):
        product_create: dict = {
            "name": "test_product",
            "description": "This is a fake test product.",
            "serial": "EumHUV41owYwmnzpjVKYng",
        ...
        res: httpx.Response = await async_client.patch(
            f"/products/{product_id}/reserve", params={"quantity": 1}
        )
        ...
        assert res.json()["data"]["quantity"] == 0

    @pytest.mark.asyncio
    async def test_out_of_stock_reserve(self, async_client: httpx.AsyncClient):
        product_create: dict = {
            "name": "test_product",
            "description": "This is a fake test product.",
            "serial": "EumHUV41owYwmnzpjVKYng",
        ...
        assert res.json()["message"] == f"Product with id {product_id} out of stock"

    @pytest.mark.asyncio
    async def test_not_enough_stock_reserve(self, async_client: httpx.AsyncClient):
        product_create: dict = {
            "name": "test_product",
            "description": "This is a fake test product.",
            "serial": "EumHUV41owYwmnzpjVKYng",
        ...
        res: httpx.Response = await async_client.patch(
            f"/products/{product_id}/reserve", params={"quantity": 5}
        )

        assert res.status_code == 400
        assert (
            res.json()["message"] == f"Product with id {product_id} quantity not enough"
        )
